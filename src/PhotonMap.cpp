...
PhotonMap::PhotonMap(std::vector<Photon> v) {
    ...
    for(int i = 1; i <= max_photons; i++) {
        ...
        bbox_min[0] = photons[i].pos.x;
        if(photons[i].pos.x > bbox_max[0])
            bbox_max[0] = photons[i].pos.x;

        if(photons[i].pos.y < bbox_min[1])
            bbox_min[1] = photons[i].pos.y;
        if(photons[i].pos.y > bbox_max[1])
            bbox_max[1] = photons[i].pos.y;

        if(photons[i].pos.z < bbox_min[2])
            bbox_min[2] = photons[i].pos.z;
        if(photons[i].pos.z > bbox_max[2])
            bbox_max[2] = photons[i].pos.z;
    }
